...
#### Seamless upgrades require latest version of Horticulturalist

Build steps were changed to support the instance monitoring endpoint. As a result, release builds now store the json of compiled design documents under a different property.  
If upgrading to `3.9.0` using an older version of Horticulturalist, the view-warming upgrade step will **not** trigger indexing for many of the views required for the server to run.   
Some of these views have changed and need to be re-indexed before being used by the application. 
View indexing will be triggered when API starts, after the upgrade is complete. This means that the server will be offline until all required views are indexed.  
Horticulturalist v.0.13.1 (latest) will trigger indexing for all views during the view-warming upgrade step, and only finalize the upgrade after all views are indexed, minimizing server downtime. 
